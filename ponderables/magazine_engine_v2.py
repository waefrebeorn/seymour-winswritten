...
def p_cover(doc, theme, date, evs, issue_no, colonel):
    ...
    for e in shown:
        ...
        body = body if len(body) <= 78 else body[:78] + "..."
        text(pg, (44, y), yr, size=11, color=accent, bold=True)
        text(pg, (92, y), body, size=9.5, color=INK)
        y += 18
    ...
    yy = by + 8
    for s in sections:
        text(pg, (50, yy), "- " + s, size=10, color=INK)
        yy += 20
